...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:Image(Config)
		...
		--// Check for URL, rbxassetid, or raw id
		if type(Config.Image) == "string" and (Config.Image:match("^http://") or Config.Image:match("^https://")) then
			-- Hỗ trợ Web URL: Tải về và Cache thẳng vào workspace của executor
			task.spawn(function()
				local writefile = writefile or (file and file.Write)
				local isfile = isfile or (file and file.Exists)
				local getcustomasset = getcustomasset or getsynasset

				if writefile and isfile and getcustomasset then
					-- Định dạng an toàn cho tên file để không cần tạo thư mục
					local ext = ".png"
					if Config.Image:match("%.jpg") or Config.Image:match("%.jpeg") then ext = ".jpg" end
					local safeName = "imgcache_" .. Config.Image:gsub("[^%w%d]", ""):sub(1, 40) .. ext
					
					if not isfile(safeName) then
						local success, result = pcall(game.HttpGet, game, Config.Image)
						if success and result then
							writefile(safeName, result)
						end
					end

					if isfile(safeName) then
						Image.Image = getcustomasset(safeName)
					end
				end
			end)
		elseif tonumber(Config.Image) then
	...
	function ContainerClass:Slider(Config)
		...
		function Config:SetValue(Value: number, IsSliderPercentage: boolean)
			...
			if not IsSliderPercentage then
		...
		------// Move events bằng UserInputService cho Mobile & PC
		local function UpdateSlider(Input)
			...
			local Percentage = (InputX - LeftPos) / SliderWidth
...
function ImGui:ApplyDraggable(Frame: Frame, Header: Frame)
	...
	Header.InputBegan:Connect(function(Key)
		if not DragInput and UserInputTypeAllowed(Key.UserInputType) then
			DragInput = Key
	...
	UserInputService.InputChanged:Connect(Movement)
end
...
function ImGui:ApplyResizable(MinSize, Frame: Frame, Dragger: TextButton, Config)
	...
	UserInputService.InputChanged:Connect(function(Input)
		if DragInput == Input then 
			...
			local mouseMoved = Vector2.new(MousePos.X - DragStart.X, MousePos.Y - DragStart.Y)

			local NewSize = OriginalSize + mouseMoved
...
function ImGui:CreateWindow(WindowConfig)
	...
	if not WindowConfig.NoDrag then
		-- Hỗ trợ Draggable trên TitleBar
